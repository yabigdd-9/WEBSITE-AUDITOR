#!/usr/bin/env python3
"""Test script to establish baseline performance using original website_auditor logic"""
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Website Rescue Auditor v2 - Baseline Test")
    ...
    async def _run():
        if args.batch:
            ...
            print(f"📊 Average time per site: {total_time/len(urls):.2f}s")
        ...
        if args.url:
            ...
            if args.format == "html":
                # Simple HTML output for now
                score = result["score"]
                color = "#FF1A1A" if score < 40 else "#FF8A00" if score < 60 else "#EFFF00" if score < 80 else "#00D4A3"
                rows = "".join(f"<tr><td>{d['defect']}</td><td>{d.get('impact','')}</td></tr>" for d in result.get("defects", []))
                output = f"""<html><head><style>body{{font-family:sans-serif;margin:2em;background:#1a1a2e;color:#eee}}
                ...
                </style></head><body><h1>{result['domain']}</h1>
                <div class="score">{score}/100</div>
                <table><tr><th>Defect</th><th>Impact</th></tr>{rows}</table>
                <p><em>Generated {result['timestamp']}</em></p></body></html>"""
