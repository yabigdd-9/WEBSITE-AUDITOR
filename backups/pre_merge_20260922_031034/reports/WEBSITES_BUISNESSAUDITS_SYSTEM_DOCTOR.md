...
Hermes capability checks pass; model execution is blocked. Ollama 0.33.3 daemon/API responds but list/ps are empty. Docker is missing, Goose is a zero-byte stub, OpenCode 1.18.29 help works. These optional states do not block deterministic auditing. OpenHands/browser-use are not required or activated. Browser verification reused installed Playwright/Chromium; no browsers or models were downloaded.
