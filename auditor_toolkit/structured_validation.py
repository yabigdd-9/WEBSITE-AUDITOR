        ...
        try:
            ...
            if isinstance(data, dict):
                if "@type" not in data:
                    findings.append(
                        Finding(
                            defect_key="structured_data_missing_type",
                        )
                    )
                ...
                for i, item in enumerate(data):
                    if not isinstance(item, dict) or "@type" not in item:
                        findings.append(
                            Finding(
                                defect_key=f"structured_data_item_missing_type_{i}",
                            )
                        )
            else:
                findings.append(
                    Finding(
                        defect_key="structured_data_not_dict_or_list",
                    )
                )
        except json.JSONDecodeError:
            findings.append(
                Finding(
                    defect_key="structured_data_invalid_json",
                )
            )
