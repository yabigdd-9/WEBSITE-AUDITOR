...
def run_audit(url, options=None, fetcher=None):
    ...
    try:
        if checks["fetch"]["status"] == "ok":
            ...
            def _hygiene():
                found = []
                robots_found, robots_data = check_robots(client, final_url)
                found.extend(robots_found)
                sitemap_found, sitemap_data = check_sitemap(
                    client, final_url, robots_data.get("sitemaps_declared") or []
                )
                found.extend(sitemap_found)
                header_found, header_data = grade_security_headers(
                    response.headers, final_url, deep=opts.deep
                )
                found.extend(header_found)
                mixed_found, mixed_data = check_mixed_content(response.text, final_url)
                found.extend(mixed_found)
                return found, {
                    "robots": robots_data,
            ...
            for name in ("page", "schema", "headers", "hygiene", "ux", "links", "tls", "dns", "crawl", "browser", "axe"):
                skip(
                    name,
                    "Fetch unavailable",
