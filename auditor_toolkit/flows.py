                ...
                if fields:
                    try:
                        ...
                        if probe.count() and probe.is_visible():
                            probe.fill("flow-probe@example.invalid", timeout=_STEP_TIMEOUT_MS)
                            ...
                            probe.fill("", timeout=_STEP_TIMEOUT_MS)
                    except Exception as exc:
...
def flow_findings(evidence: dict, url: str, status: str = "ok"):
    ...
    if status == "error" or evidence.get("status") == "error":
        findings.append(
            Finding(
                "flow-probe-failed",
